...
describe('/passwordreset', () => {
    ...
    describe("POST", () => {
        ...
        test("if account exists for the email return a 200 with success and reset token record is created", async () => {
            ...
            //
            const res = await app.request("/passwordreset", {
                method: "POST",
                headers: {
    ...
    describe("PATCH", () => {
        ...
        test("request with invalid token returns the invalid message", async () => {
            ...
            //
            const res = await app.request(`/passwordreset/${crypto.randomUUID()}`, {
                method: "PATCH",
                headers: {
                    "content-type": "application/json"
                }
        ...
        test("request with valid but expired token returns the invalid message", async () => {
            ...
            //
            const res = await app.request(`/passwordreset/${resetToken.tokenId}`, {
                method: "PATCH",
                headers: {
                    "content-type": "application/json"
                }
